...
- partnerName: 取引先名。法人格（株式会社・有限会社・合同会社等）を含む正式名称。
  ★重要★ これは「証憑を発行した側＝あなたに請求/領収した相手企業」です（請求書なら請求元、領収書なら発行店）。
  会社印・登録番号・振込口座名義の近くの会社名が該当。「〇〇 御中/様」の宛先（受取側＝自社/自団体）は絶対に選ばないこと。
